...
class addwin():
    def __init__(self, master):
        ...
        self.r1 = ttk.Radiobutton(self.addwin, text="单本入库（或修改信息）", variable=self.mode, value=1, command=self.display1,
                                  style="radio.TRadiobutton")
        self.r2 = ttk.Radiobutton(self.addwin, text="批量入库", variable=self.mode, value=2, command=self.display2,
                                  style="radio.TRadiobutton")
    ...
    def display2(self, ):
        ...
        self.f_prom = ttk.Label(self.addwin, text="请输入文件名字:", font=('微软雅黑', 12), width=70, style="label.TLabel",
                                anchor="center")
...
class querywin():
    ...
    def query(self, index):
        ...
        if index != 3 and index != 5:
            self.search_prom1 = ttk.Label(self.querywin, text="请输入 " + self.search_str + ":", style="l.TLabel",
            ...
            # pass
            self.search_prom2 = ttk.Label(self.querywin, text="请输入 " + self.search_str + ':', style="l.TLabel",
    ...
    def show(self, result):
        ...
        self.query_list = ttk.Treeview(self.querywin, columns=["书号", "类别", "书名", "出版社", "年份", "作者", "价格", "库存数"],
                                       show="headings")  # show设为headings可以隐藏首列
...
class cardwin():
    def __init__(self, master):
        ...
        self.r1 = ttk.Radiobutton(self.cardwin, text="增添或修改", variable=self.mode, value=1, command=self.Add,
                                  style="r.TRadiobutton")
        self.r2 = ttk.Radiobutton(self.cardwin, text="删除借书证", variable=self.mode, value=2, command=self.Delete,
                                  style="r.TRadiobutton")
    ...
    def Add(self):
        ...
        self.type_input1 = ttk.Radiobutton(self.cardwin, text="老师", variable=self.type_input, value=1,
                                           command=self.T_type, style="r1.TRadiobutton")
        self.type_input2 = ttk.Radiobutton(self.cardwin, text="学生", variable=self.type_input, value=2,
                                           command=self.S_type, style="r1.TRadiobutton")
...
class borrowwin():
    ...
    def confirm(self):
        ...
        if type(borrow_list) == type(()):
            self.query_list = ttk.Treeview(self.borrowwin, columns=["书号", "书名", "借书时间", "归还期限", "是否可以续借"],
                                           show="headings")  # show设为headings可以隐藏首列
            self.query_list.column("书号", width=100)
            self.query_list.column("书名", width=200)
...
class returnwin():
    ...
    def confirm(self):
        ...
        if type(borrow_list) == type(()):
            self.query_list = ttk.Treeview(self.returnwin, columns=["书号", "书名", "借书时间", "归还期限", "是否可以续借"],
                                           show="headings")  # show设为headings可以隐藏首列
            self.query_list.column("书号", width=100)
            self.query_list.column("书名", width=200)
            self.query_list.column("借书时间", width=200)
